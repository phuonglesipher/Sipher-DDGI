...
// ============================================================================
// Inline Path Tracing Functions
// ============================================================================

/**
 * Trace a ray using inline ray tracing and shade the hit point.
 * Returns true if a surface was hit, false if the ray missed (sky).
 */
bool TraceAndShadeInline(
    RayDesc ray,
    ...
    out Payload payload)
{
    ...
    // Miss - set hitT to negative to indicate miss
    payload.hitT = -1.f;
    return false;
}

/**
 * Trace a path through the scene using inline ray tracing.
 */
float3 TracePathInline(RayDesc ray, uint seed, RaytracingAccelerationStructure SceneTLAS, StructuredBuffer<Light> Lights)
{
    ...
    {
        // Trace the ray using inline ray tracing
        Payload payload;
        bool hit = TraceAndShadeInline(ray, SceneTLAS, payload);

        // Miss, add sky radiance and exit loop
        if (!hit)
        {
        ...
        // Direct Lighting using inline visibility rays
        float3 diffuse = DirectDiffuseLightingInline(payload, GetGlobalConst(pt, rayNormalBias), GetGlobalConst(pt, rayViewBias), SceneTLAS, Lights);
...
void CS(uint3 GroupID : SV_GroupID, uint3 GroupThreadID : SV_GroupThreadID, uint3 DispatchThreadID : SV_DispatchThreadID)
{
    ...
    PTOutput.GetDimensions(LaunchDimensions.x, LaunchDimensions.y);
    ...
    for (int sampleIndex = 0; sampleIndex < GetPTSamplesPerPixel(); sampleIndex++)
    {
        ...
        // Trace path using inline ray tracing
        color += TracePathInline(ray, seed, SceneTLAS, Lights);
    }
