...
void CS(uint3 GroupID : SV_GroupID, uint3 GroupThreadID : SV_GroupThreadID, uint3 DispatchThreadID : SV_DispatchThreadID)
{
    ...
    // Trace using inline ray tracing
    ...
    if (RQuery.CommittedStatus() == COMMITTED_TRIANGLE_HIT)
    {
        ...
        uint instanceIndex = RQuery.CommittedInstanceID();
        float3 worldPosition = ray.Origin + ray.Direction * hitT;
        ...
        if (depth < 0.f || hitT < depth)
        {
            // Get the DDGIVolume index from the instance's custom data
            // For now, assume volume index is 0 (first volume)
            uint volumeIndex = 0;
            ...
            int probeIndex = (instanceIndex - instanceOffset);
            ...
            float3 color = GetProbeData(probeIndex, probeCoords, worldPosition, resourceIndices, volume, sampleDirection);
            ...
            if (volume.probeClassificationEnabled)
            {
                ...
                // Probe coloring
                if (abs(dot(normalize(ray.Direction), sampleDirection)) < 0.45f)
                {
            ...
            WriteResult(LaunchIndex, color, hitT, GBufferA, GBufferB);
        }
    }
}

[numthreads(8, 8, 1)]
void CS_HideInactive(uint3 GroupID : SV_GroupID, uint3 GroupThreadID : SV_GroupThreadID, uint3 DispatchThreadID : SV_DispatchThreadID)
{
    uint2 LaunchIndex = DispatchThreadID.xy;
    uint2 LaunchDimensions;
    ...
    // Use a loop to find active probes (skip inactive ones)
    float currentTMin = 0.f;
    const int MAX_ITERATIONS = 16;  // Limit iterations to prevent infinite loops

    for (int iteration = 0; iteration < MAX_ITERATIONS; iteration++)
    {
        ray.TMin = currentTMin;
        ...
        RQuery.TraceRayInline(
            DDGIProbeVisTLAS,
            RAY_FLAG_CULL_BACK_FACING_TRIANGLES,
            0x02,  // Instance mask for hide-inactive mode
            ray);
        RQuery.Proceed();
        ...
        // Update TMin for next iteration
        currentTMin = hitT + 0.001f;
        ...
        if (depth < 0.f || hitT < depth)
        {
            ...
            // Early out: if the probe is inactive, continue to next hit
            uint3 probeStateTexCoords = DDGIGetProbeTexelCoords(probeIndex, volume);
            float probeState = ProbeData[probeStateTexCoords].w;
            ...
            // Found an active probe, exit the loop
            break;
        }
    }
}
